/* file: spi_and_ser_v1.4.1.ino 
currently, the ADC hardware does not work, so there is no point polling it. */
...
uint16_t ADC_read(int channel) {
    ...
    uint16_t ret = ((((uint16_t)(b1)) << 8) | b2) >> 1;   //combine the bytes and get rid of blank first bit (LSB)
    return ret;
}
...
uint16_t execute_channel_cmd(uint8_t cmdBytes[]) {
  ...
  uint16_t cmdData = (((uint16_t)(cmdBytes[1])) << 4) + (cmdBytes[2] >> 4);
  ...
  int start_addr = (addr > 0 ? addr : 1);  // addr 0 is global; address values 1-NCHANNELS are valid.
  int stop_addr = (addr > 0 ? addr + 1 : NCHANNELS + 1);
    ...
    for (int i=start_addr; i<stop_addr; i++) {
        ...
        channelList[i-1].enable = en;               
        if (upd) {  /* set DAC value */
            DAC_write(i, cmdData);
            channelList[i-1].DAC_val = cmdData;
            retVal = cmdData;
        }            
        set_direction(i, dir);   /* set H-brige direction */
        channelList[i-1].dir = dir;
    }
    return retVal;
}
...
void setup() {
    ...
    nextByte = 0;
    newPacket = 1;
    noCmds = 1; // have not received any commands yet
          ...
          if (packetIndex == ((packetBytes[PCK_LEN(ncmds)-PCK_CHK_LEN-1]) >> 5) & 7) {
            if (fletcher8(PCK_LEN(ncmds)-1, packetBytes) == packetBytes[PCK_LEN(ncmds)-1]) {
              ...
              for (int i=0; i<ncmds; i++) {
                ...
                //uint16_t cmdRet = (((int)(cmdBytes[1])) << 4) + (cmdBytes[2] >> 4); // echo back the command data.                
