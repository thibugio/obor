...
void DAC_write(int channel, uint16_t to_dac) {
    ...
    word data = highByte(to_dac);
    data &= 0b00001111;
    data = data | DAC_SELECT | INPUT_BUF | GAIN_SELECT | PWR_DOWN;
    SPI.transfer(data);
    data = lowByte(to_dac);
    SPI.transfer(data);
...
void toggle_enable(int channel, int en) {
    if (en == 0) {  // disable 
        digitalWrite(H_Enable_pins[channel], HIGH);
    } else {  // enable 
        digitalWrite(H_Enable_pins[channel], LOW);
    }
}
...
void set_direction(int channel, int direction) {
    if (direction == 0) {
        digitalWrite(H_Neg_pins[channel], HIGH);
        digitalWrite(H_Pos_pins[channel], LOW);
    } else {
        digitalWrite(H_Pos_pins[channel], HIGH);
        digitalWrite(H_Neg_pins[channel], LOW);
    }
}
