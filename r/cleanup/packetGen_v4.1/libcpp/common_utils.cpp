...
std::vector<uint8_t> compactPreambleBytes(int cmdIndex, int ncmds) {
    std::vector<uint8_t> bytes;
    int i;
    for (i=0; i<PRE_LEN; i++) {
        if (i==0) {
            bytes.push_back(PCK_OK << 7);          /* ok1 */
            bytes[i] |= (cmdIndex & 7) << 4;    /* index3 */
...
std::vector<uint8_t> compactPostambleBytes(int cmdIndex) {
    ...
    for (i=0; i<POST_LEN; i++) {
        if (i==0) {
            bytes.push_back(cmdIndex << 5);  // index3
...
std::vector<uint8_t> compactPacketBytes(std::vector<catheterChannelCmd>& cmdVect, int cmdIndex) {
    std::vector<uint8_t> bytes;
    int ncmds = cmdVect.size();
    
    std::vector<uint8_t> preV = compactPreambleBytes(cmdIndex, ncmds);
    std::vector<uint8_t> cmdV = compactCommandVectBytes(ncmds, cmdVect);
    std::vector<uint8_t> postV = compactPostambleBytes(cmdIndex);
...
void getPacketBytes(std::vector<catheterChannelCmd>& commandVect, std::vector<std::vector<uint8_t>>& pbytes, 
    ...
    for(int i = 0; i < ncmds; i++) {
        ...
        if (commandVect[i].waitTime > 0 || i==(ncmds-1)) {
            pbytes.push_back(compactPacketBytes(tempV, cmdIndex));
