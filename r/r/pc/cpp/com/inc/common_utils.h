...
struct catheterChannelCmd {
    ...
    //unsigned int chksum4;   /* checkum for individual command: 4 bits */
...
struct strictCatheterPacket {
    uint8_t pseqnum;
    unsigned long delay;
    catheterChannelCmd cmds[NCHANNELS];
};
...
catheterChannelCmd resetCommand(void);
