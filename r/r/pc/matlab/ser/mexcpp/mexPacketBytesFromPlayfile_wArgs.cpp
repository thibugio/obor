/* mexcpp/mexPacketBytesFromPlayfile.cpp */

#include "common_utils.h"
#include "pc_utils.h"
#include "mex_utils.h"

//README: experiment with using system call in macro to get cwd and reference playfile from there
//#define PLAYPATH "/home/acceber/common/workspaces/matlab/cwru/research/cavusoglu/catheter/matlabPacketGen_v4.3/play/catheter.play"
//#define PLAYPATH "C:/Users/acceber/Documents/MATLAB/cwru/research/cavusoglu/catheter/matlabPacketGen_v4.3/play/catheter.play"


using namespace std;

/* for matlab-- populate a vector of packet bytes and a vector of inter-packet wait-times */
void mexPacketBytesFromPlayfile(std::vector<std::vector<uint8_t>>& packetVect, std::vector<int>& waits, const char * path) {
    packetVect.clear();
    waits.clear();    
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs != 1) {
    mexErrMsgIdAndTxt("MATLAB:mexPacketBytesFromPlayfile_wArgs:nargin", 
            "getPacketStringsFromPlayfile requires 1 input arguments.");
  } else if (nlhs != 3) {
    mexErrMsgIdAndTxt("MATLAB:mexPacketBytesFromPlayfile_wArgs:nargout",
            "getPacketStringsFromPlayfile requires 3 output arguments.");
  }
  
  /* check that input arg is a string */
  if ( mxIsChar(prhs[0]) != 1) {
      mexErrMsgIdAndTxt( "MATLAB:mexPacketBytesFromPlayfile_wArgs:inputNotString",
  ...
  const char * path = mxArrayToString(prhs[0]);
  
  mexPacketBytesFromPlayfile(packetVect, waits, data, path);
